...
class RollbarApi {
  ...
  async get(url, accessToken) {
    ...
    return axios2.get(fullUrl, getRequestHeaders(accessToken || this.accessToken || "")).then((response) => {
      var _a;
      return camelcaseKeys((_a = response == null ? void 0 : response.data) == null ? void 0 : _a.result, {deep: true});
    });
  }
...
function getRollbarAccountToken(logger) {
  const token = process.env.ROLLBAR_ACCOUNT_TOKEN || "";
  if (!token) {
    if (process.env.NODE_ENV !== "development") {
      throw new Error("Rollbar token must be provided in ROLLBAR_ACCOUNT_TOKEN environment variable to start the API.");
    }
    logger.warn("Failed to initialize rollbar backend, set ROLLBAR_ACCOUNT_TOKEN environment variable to start the API.");
  }
  return token;
}
