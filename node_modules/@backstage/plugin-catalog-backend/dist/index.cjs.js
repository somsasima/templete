...
const migrationsDir = path2.resolve(require.resolve("@backstage/plugin-catalog-backend/package.json"), "../migrations");
...
class GithubApiReaderProcessor {
  constructor() {
    this.privateToken = process.env.GITHUB_PRIVATE_TOKEN || "";
  }
  getRequestOptions() {
    const headers = {
...
class LocationReaders {
  static defaultProcessors(entityPolicy = new catalogModel.EntityPolicies()) {
    return [
  ...
  constructor(logger = backendCommon.getVoidLogger(), processors = LocationReaders.defaultProcessors()) {
    this.logger = logger;
