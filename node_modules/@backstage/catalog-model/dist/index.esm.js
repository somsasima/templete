...
const DEFAULT_ENTITY_SCHEMA = object({
  apiVersion: string().required(),
  kind: string().required(),
  metadata: object({
    uid: string().notRequired().min(1),
    etag: string().notRequired().min(1),
    generation: number().notRequired().integer().min(1),
    name: string().required(),
    namespace: string().notRequired(),
    description: string().notRequired(),
    labels: object().notRequired(),
    annotations: object().notRequired()
  }).required(),
  spec: object({}).notRequired()
});
...
class ComponentEntityV1alpha1Policy {
  constructor() {
    this.schema = object({
      apiVersion: string().required().oneOf(API_VERSION),
      kind: string().required().equals([KIND]),
      spec: object({
        type: string().required().min(1),
        lifecycle: string().required().min(1),
        owner: string().required().min(1)
      }).required()
    });
  }
...
const KIND$1 = "Location";
class LocationEntityV1alpha1Policy {
  constructor() {
    this.schema = object({
      apiVersion: string().required().oneOf(API_VERSION$1),
      kind: string().required().equals([KIND$1]),
      spec: object({
        type: string().required().min(1),
        target: string().notRequired().min(1),
        targets: array(string()).notRequired()
      }).required()
    });
  }
  async enforce(envelope) {
...
const KIND$2 = "Template";
class TemplateEntityV1alpha1Policy {
  constructor() {
    this.schema = object({
      apiVersion: string().required().oneOf(API_VERSION$2),
      kind: string().required().equals([KIND$2]),
      spec: object({
        type: string().required().min(1),
        path: string(),
        schema: object().required(),
        templater: string().required()
      }).required()
    });
  }
  async enforce(envelope) {
...
export { CommonValidatorFunctions, ComponentEntityV1alpha1Policy, DefaultNamespaceEntityPolicy, EntityPolicies, FieldFormatEntityPolicy, KubernetesValidatorFunctions, LOCATION_ANNOTATION, LocationEntityV1alpha1Policy, NoForeignRootFieldsEntityPolicy, ReservedFieldsEntityPolicy, SchemaValidEntityPolicy, TemplateEntityV1alpha1Policy, entityHasChanges, entityMetaGeneratedFields, generateEntityEtag, generateEntityUid, generateUpdatedEntity, locationSchema, locationSpecSchema, makeValidator };
